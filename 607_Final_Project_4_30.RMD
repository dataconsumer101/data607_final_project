...
# Let's use the same function we created earlier to unpivot year columns
dtp_rate_tall <- unpivot(m_rate, 'dtp_rate')
MCV_rate_tall <- unpivot(s_rate, 'MCV_rate')
PAB_rate_tall <- unpivot(m_rate, 'PAB_rate')
hepb3_rate_tall <- unpivot(s_rate, 'hepb3_rate')
dfVax <- pv_df %>%
...
summary(dfVax)
```
Looking at the summary, the quantity of NAs, each of these have a fairly high and similar quantity of responses. Additonally, it seems like two of these may be repeated datasets. It is unclear if this is an error in data storage or collection, however, for the purposes of this project, we will focus on measels and DTP. 
```{r}
df <- df %>%
  full_join(dtp_rate_tall, by = c('country' = 'country', 'year' = 'year')) %>%
  full_join(MCV_rate_tall, by = c('country' = 'country', 'year' = 'year'))
