...
LowVaxCounties <- top_n(Vaccine_Sum, 20, -Avg_Avg) 
LowVaxCounties

dtp_rate_tall %>%
  filter(country %in% LowVaxCounties$Country) %>%
ggplot(aes(x = year, y = dtp_rate, color = country)) +
  geom_line() +
  theme_bw() + 
  theme(legend.position = 'none') +
  labs(title = 'Dtp Vaccination Rate by Country',
       x = element_blank(),
       subtitle = 'Filtered for 20 lowest countries') 
...
Similiarly, dtp and MCV vaccine rates are very correlated. These vaccines became widely availiable around the same time, and their acceptance and usuage is largely colinear. 

Looking toward some less anticpated corrleations, there is a rather strong correlation between vaccine rate and primary school completion rate. Countries with a well organized public school system are more likely to make vaccinations compulsary. While it is true that the vaccination rate relates to vaccines given to childern in their first year of life - it is not hard to imagine that vaccination requirements in public school is an indication of a broder acceptance of vaccinations in that society.

Though unrelated to our predictor variables, the strongest correlation to the inequality index is sucide rate. Interestingly enough, there's a weak correlation between adult literacy rate and the suicide rate. It's a bit scary to think that there's a link between being able to read and suicide. There's also a weak negative correlation between the primary school completion rate and the murder rate.
